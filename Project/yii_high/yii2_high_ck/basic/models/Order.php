...
class Order extends ActiveRecord
{
	...
	public static function getData($order)
	{
		...
		->where('orderid = :oid', [':oid' => $order->orderid])
		->all();
		$products = [];
		...
		$user = User::find()
		->where('userid = :uid', [':uid' => $order->userid])
		->one();
		if (!empty($user)) {
			$order->username = $user->username;
		}
		$order->address = Address::find()
		->where('addressid = :aid', [':aid' => $order->addressid])
		->one();
	...
	public static function getProducts($userid)
	{
		...
		foreach ($orders as $order) {
			...
			foreach ($details as $detail) {
				...
				$product->cate = Category::find()
				->where('cateid = :cid', [':cid' => $product->cateid])
				->one()
				->title;
				$products[] = $product;
			}
