...
class User extends \yii\base\Object implements \yii\web\IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            ['usermail', 'required', 'message' => '电子邮件不能为空', 'on' => ['reg', 'regbymail']],
            ['usermail', 'email', 'message' => '电子邮件格式不正确', 'on' => ['reg', 'regbymail']],
            ['usermail', 'unique', 'message' => '电子邮件已经被注册', 'on' => ['reg','regbymail']],
    ...
    {
        return self::find()->where('username = :loginname or usermail = :loginname', [':loginname' => $this->loginname])->one();
    }
    ...
    public function regByMail($data)
    {
        ...
        if ($this->load($data) && $this->validate()) {
            ...
            $mailer->setTo($data['User']['usermail']);
            $mailer->setSubject('慕课商城-新注册用户');
