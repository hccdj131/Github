...
class Admin extends ActiveRecord implements \yii\web\IdentityInterface
{
	...
	{
		return self::find()
		->where('adminuser = :user', [':user' => $this->adminuser])
		->one();
	}
	...
	public function seekPass($data)
	{
		...
		if ($this->load($data) && $this->validate()) {
			...
			$mailer->setFrom("hccdj131@163.com");
	...
	public function reg($data)
	{
		...
		if ($this->load($data) && $this->validate()) {
			...
			$this->adminpass = Yii::$app->getScurity()->generatePasswordHash($this->adminpass); // 把adminpass传进去
			if ($this->save(false)) {
				return true;
			}
			return false;
